...
玩家 **@机器人 + 曲名或别名** 进行回答，在 **最大猜测次数**（`max_guesses`，默认 12，运行时限制为 1-20）内猜出目标曲目（未 @ 机器人的消息不参与游戏）。匹配规则：
...
**计分**：按最大猜测次数四等分，由快到慢得 4/3/2/1 分（默认 12 次时：第 1-3 次得 4 分，4-6 次得 3 分，7-9 次得 2 分，10-12 次得 1 分）。
支持奖励有效时间（`reward_valid_time`，默认 3 秒，0 为禁用）：首位玩家猜中后，在奖励有效时间内答对的其他玩家也能获得相同分数。
